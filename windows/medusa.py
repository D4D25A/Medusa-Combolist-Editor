...
user_profile = home+'\\AppData\\Roaming\\Medusa'
    ...
    if localedithwid == localhwid and localedituser == username:
        ...
        def localeditone():
            ...
            def emailpassuhq():
                ...
                print(centered)
                if localedithwid == localhwid and localedituser == username:
                    etulogged = align_center(f" Authenticated under, {bcolors.CYAN}"+username+f".\n{bcolors.ENDC}", 105)
                    print(etulogged)
                    time.sleep(1)
                else:
...
def encryptionsettings():
    ...
    print(f"{bcolors.CYAN}[{bcolors.ENDC}B{bcolors.CYAN}]{bcolors.ENDC} Go to Local Combolist Edit Options")
    print(f"{bcolors.CYAN}[{bcolors.ENDC}M{bcolors.CYAN}]{bcolors.ENDC} Back to Main Menu")
    print(f"\n{bcolors.OKGREEN}What would you like to do today? {bcolors.ENDC}"); encryptionpreference = (msvcrt.getch()).decode("utf-8")
...
def mainui():
    os.system('cls' if os.name == 'nt' else 'clear')
    print(centered)
    welcome = align_center(f" Welcome back, {bcolors.CYAN}"+username+f".{bcolors.ENDC}", 105)
    print(welcome)
    member = align_center(f"    Your current plan: {bcolors.FAIL}"+membership+f"{bcolors.ENDC} \n   You have access to: {bcolors.WARNING}"+access+f".\n{bcolors.ENDC}", 105)
    print(member)
    time.sleep(1)
...
centered = align_center(text, 100)
print(centered)
version = "1.0.0"
...
if version == versionid:
    verified = align_center(" Medusa is up-to-date, v"+version+"\n", 100)
...
while loop:
    ...
    if localhwid == hwid:
        suc = align_center(f"\n{bcolors.OKGREEN}Authentication successful...{bcolors.ENDC}", 110)
        for char in suc:
            time.sleep(0.010)
            sys.stdout.write(char)
            sys.stdout.flush()
